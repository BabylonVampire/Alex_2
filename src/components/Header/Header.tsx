    ...
    <div className={styles.header}>
      ...
      >
        <div className={styles.burgerLinks}>
          {links.map((link) => {
            return (
              <a key={link.link + link.heading} className={styles.link} href={link.link} onClick={changeBurger}>
                {link.heading}
              </a>
            );
          })}
        </div>
        <div className={styles.burgerPhone}>+7(903)718-22-79</div>
      </div>
      <div className={styles.pcDisplay}>
        ...
        <div className={styles.linkBox}>
          {links.map((link) => {
            return (
              <a className={styles.link} href={link.link} key={`burger${link.link + link.heading}`}>
                {link.heading}
              </a>
            );
          })}
